...
//shreyash
    if(isset($_POST['login'])){
        ...
        $sql = "SELECT * FROM studentadmins WHERE username=? AND password=? AND branch=? AND Year_Of_Study=? AND division=?";
        $stmt = $connection->prepare($sql);
        $stmt->bind_param("sssss",$username,$password,$branch,$Year_Of_Study,$division);
        $stmt->execute();
                ...
                <form action= "<?= $_SERVER['PHP_SELF'] ?>" method="post" class="p-4">
                    ...
                    <div class="form-group">
                            ...
                            <select name="Year_Of_Study" class="form-control" required>
                                <option name="Year_Of_Study" value="">-- SELECT BRANCH --</option>
