...
if(isset($_POST['password_reset_link']))
{
    ...
    $check_email = "SELECT email FROM superadmin WHERE email='$email' LIMIT 1";
    ...
    if(mysqli_num_rows($check_email_run) > 0)
    {
         ...
         $update_token = "UPDATE superadmin SET verify_token='$token' WHERE email='$get_email' LIMIT 1";
...
if(!empty($token))
{
    if(!empty($token) && !empty($new_password) && !empty($confirm_password))
    {
        ...
        $check_token = "SELECT verify_token FROM superadmin WHERE verify_token='$token' LIMIT 1";
        ...
        if(mysqli_num_rows($check_token_run) > 0)
        {
               ...
               {   
                // $new_password = sha1($new_password);
                   $update_password = "UPDATE superadmin SET password='$new_password' WHERE verify_token='$token' LIMIT 1";
