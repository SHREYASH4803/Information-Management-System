...
<?php 
    if (isset($_POST["submit"])) {
        ...
        $sth = "SELECT * FROM `internship_details` WHERE user_id= '$id' AND (Branch LIKE '%$str%' OR Roll_no LIKE '%$str%' OR Name_Of_The_Student LIKE '%$str%' OR Year_Of_Study LIKE '%$str%' OR academic_year LIKE '%$str%' OR Name_Of_Organization LIKE '$str' OR Number_of_Days_of_Internship LIKE '%$str%' OR Organizing_Addr LIKE '%$str%' OR Dates_From LIKE '%$str%' OR Dates_To LIKE '%$str%' OR STATUS LIKE '$str' ) ";
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.editbtn').on('click', function () {
                ...
                $('#Dates_To').val(data[6]);
                $('#Number_of_Days_of_Internship').val(data[7]);
                $('#Name_of_Organization').val(data[8]);
