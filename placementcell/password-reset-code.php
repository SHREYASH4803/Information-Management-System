...
function send_password_reset($get_username,$get_email,$token)
{
    ...
    <br/>
    <a href='http://localhost/PramodGarhwal/ims/professors/password-change.php?token=$token&email=$get_email'>Click me </a>
    ";
...
if(isset($_POST['password_reset_link']))
{
    ...
    $check_email = "SELECT email FROM placementcell WHERE email='$email' LIMIT 1";
    ...
    if(mysqli_num_rows($check_email_run) > 0)
    {
         ...
         $update_token = "UPDATE placementcell SET verify_token='$token' WHERE email='$get_email' LIMIT 1";
...
if(!empty($token))
{
    if(!empty($token) && !empty($new_password) && !empty($confirm_password))
    {
        ...
        $check_token = "SELECT verify_token FROM placementcell WHERE verify_token='$token' LIMIT 1";
        ...
        if(mysqli_num_rows($check_token_run) > 0)
        {
               ...
               {   
                // $new_password = sha1($new_password);
                   $update_password = "UPDATE placementcell SET password='$new_password' WHERE verify_token='$token' LIMIT 1";
