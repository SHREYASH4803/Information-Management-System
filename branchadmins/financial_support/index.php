...
<div id="tabledataid" class="card">
    <div class="card-body">
        ...
        <table id="datatableid" class="table table-bordered table-dark mt-2">
            ...
            $table_query = "SELECT * FROM financial_support WHERE branch = '$branch' ORDER BY id ASC";
        ...
        if (isset($_POST['approve'])) {
            $id = $_POST['id'];
            $select = "UPDATE financial_support SET STATUS ='APPROVED' WHERE id='$id'";
        ...
        if (isset($_POST['reject'])) {
            $id = $_POST['id'];
            $select = "UPDATE financial_support SET STATUS ='Sent Back' WHERE id='$id'";
            $result = mysqli_query($connection, $select);
            echo "Data Rejected";
            header("Location: index.php");
        }
...
<?php 
    if (isset($_POST["submit"])) {
        ...
        $sth = "SELECT * FROM `financial_support` WHERE branch = '$branch' AND (Academic_Year LIKE '%$str%' OR Name_Of_The_Teacher LIKE '%$str%' OR Title_Of_Program LIKE '%$str%' OR Activity_Name LIKE '%$str%' OR Other_Activity LIKE '$str' OR Dates_From LIKE '%$str%' OR Dates_To LIKE '%$str%' OR Name_Of_Activity LIKE '%$str%' OR Amount_Of_Support LIKE '%$str%' OR STATUS LIKE '$str')";
