...
<div id="tabledataid" class="card">
    <div class="card-body">
        ...
        if (isset($_POST['reject'])) {
            $id = $_POST['id'];
            $select = "UPDATE fdpsttpattendedit SET STATUS ='Sent Back' WHERE id='$id'";
            $result = mysqli_query($connection, $select);
            echo "Data Rejected";
            header("Location: index.php");
        }
...
<?php 
    if (isset($_POST["submit"])) 
    {
        ...
        $sth = "SELECT * FROM `fdpsttpattendedit` WHERE branch = '$branch' AND ( other LIKE '%$str%' OR Academic_Year LIKE '%$str%' OR Name_Of_The_Teacher LIKE '%$str%' OR Branch LIKE '%$str%' OR Title_Of_Program LIKE '%$str%' OR Professional_Body_Or_Organization_Associated LIKE '%$str%' OR Course_Type LIKE '%$str%'  OR Organizing_Institute_And_Location LIKE '%$str%' OR Duration LIKE '%$str%' OR STATUS LIKE '$str') ";
